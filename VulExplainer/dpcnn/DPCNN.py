...
class DPCNN(nn.Module):
    ...
    def __init__(
        ...
        self.groups_head = nn.Linear(self.dim_channel, 5)
    def _forward(self, x):
    ...
    def forward(self, input_ids,groups, labels, fgsm_attack=False, lambda_fgsm=0.1):
        ...
        # # 首先预测groups
        # groups_logits = self.groups_head(features)  # shape: [batch_size, num_groups]
        # if groups is None:
        #     groups_probs = F.softmax(groups_logits, dim=1)  # shape: [batch_size, num_groups]
        #     groups = groups_probs.argmax(dim=1) 

        # groups_one_hot = F.one_hot(groups, num_classes=groups_logits.shape[1]).float()
