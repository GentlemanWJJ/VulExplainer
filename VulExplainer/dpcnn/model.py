...
class Model(nn.Module):
    ...
    def forward(self, input_ids,groups, labels, fgsm_attack=True):
        ...
        features = self.dpcnn(embeddings)
        ...
        # 对抗样本前向
        adv_features = self.dpcnn(adv_embeddings)
        adv_logits = self.fc_labels(adv_features)
        adv_groups_logits = self.fc_groups(adv_features)
        ...
        loss = (1 - self.lambda_fgsm) * 0.5 * (loss + loss_adv) + self.lambda_fgsm * loss_ctr
        return loss
